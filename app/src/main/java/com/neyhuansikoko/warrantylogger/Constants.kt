package com.neyhuansikoko.warrantylogger

const val IMAGE_DIR = "images"

const val TEMP_IMAGE_SUFFIX = ".jpg"
//const val COMPRESSION_QUALITY: Int = 70

const val THUMBNAIL_SUFFIX = "-thumbnail"
const val THUMBNAIL_SIZE = 150
const val NO_NAME_IMAGE = "missing-name"

const val NOTIFICATION_ID: Int = 37

const val DEFAULT_DURATION: Int = 2

const val FILENAME_FORMAT = "yyyy-MM-dd-HH-mm-ss-SSS"

const val FLASH_MODE_KEY = "FLASH_MODE_KEY"
const val SETTINGS_LOCK_KEY = "SETTINGS_LOCK_KEY"

const val DATABASE_NAME = "warranty-logger-app-database"
const val WAL_FILE_SUFFIX = "-wal"
const val SHM_FILE_SUFFIX = "-shm"
const val BACKUP_FILE_SUFFIX = "-backup"

const val BACKUP_ZIP = "warranty-logger-backup.zip"

const val ZIP_CONTENT_TYPE = "application/zip"